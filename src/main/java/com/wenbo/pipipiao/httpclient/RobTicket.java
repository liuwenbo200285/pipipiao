...
public class RobTicket {
	...
	public RobTicket(ConfigInfo configInfo, UserInfo userInfo,
			Map<String, UserInfo> userInfoMap, HttpClient httpClient) {
	...
	public void getLoginRand() {
		...
		try {
			...
			logger.info("close getLoginRand response");
			HttpClientUtils.closeQuietly(response);
		}
	}

	public void login(String loginRand, String randError)
		...
		try {
			...
			parameters.add(new BasicNameValuePair("user.password", configInfo
					.getUserpass()));
			...
			logger.info("close login response!");
			HttpClientUtils.closeQuietly(response);
		}
	}
	...
	public String getRandCode(UrlEnum urlEnum) {
		HttpGet httpGet = new HttpGet("https://dynamic.12306.cn/otsweb/"
				+ urlEnum.getPath());
		...
		try {
			...
			if (response.getStatusLine().getStatusCode() == 200) {
				...
				String fileDir = this.getClass().getClassLoader()
						.getResource(".").getPath();
